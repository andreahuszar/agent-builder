...
\restrict TSMMs6cGuUGZhgpSfCTElqTgbDdbcwbmdeaZhOy1tcWqiSc45YpbWmujJSjmidr
...
CREATE VIEW public.invoice_headers_enriched AS
 SELECT ih.id,
    ...
    ih.tax_total,
    ih.shipping_total,
    ih.other_charges_total,
    ih.withholding_tax_total,
    ih.rounding_diff,
    ih.total,
    ih.paid_amount,
    ih.payment_terms_id,
    ...
    ih.tax_inclusive,
    ih.tax_treatment_code,
    ih.self_billed,
    ih.early_pay_discount_offered_percent,
    ih.early_pay_discount_deadline,
    ih.early_pay_discount_taken_amount,
    ih.prepayment_reference,
    ih.references_invoice_id,
    ih.revision,
    ih.supersedes_invoice_id,
    ih.fingerprint_sha256,
    ih.status,
    ih.match_status,
    ih.hold_reason,
    ih.hold_reason_code,
    ...
    (ih.due_date - CURRENT_DATE) AS days_until_due,
        ...
        END AS days_overdue,
        CASE
            WHEN ((ih.early_pay_discount_deadline IS NOT NULL) AND (CURRENT_DATE <= ih.early_pay_discount_deadline) AND (ih.early_pay_discount_offered_percent IS NOT NULL)) THEN ((ih.total * ih.early_pay_discount_offered_percent) / (100)::numeric)
            ELSE (0)::numeric
...
CREATE VIEW public.match_results_summary AS
 SELECT mr.invoice_id,
    ...
    mode() WITHIN GROUP (ORDER BY mr.explanation_code) AS most_common_issue,
        CASE
            WHEN (count(*) FILTER (WHERE (mr.within_tolerance = false)) = 0) THEN 'APPROVE'::text
            WHEN (count(*) FILTER (WHERE (mr.within_tolerance = false)) <= 2) THEN 'REVIEW'::text
            ELSE 'REJECT'::text
...
CREATE VIEW public.work_queue AS
 SELECT wi.id AS work_item_id,
    ...
    wi.updated_at,
        CASE
            WHEN (wi.doc_type = 'INV'::public.doc_type) THEN ( SELECT ih.invoice_number
               FROM public.invoice_headers ih
              WHERE (ih.id = wi.doc_id))
            WHEN (wi.doc_type = 'PO'::public.doc_type) THEN ( SELECT po.po_number
               FROM public.po_headers po
              WHERE (po.id = wi.doc_id))
            WHEN (wi.doc_type = 'GR'::public.doc_type) THEN ( SELECT gr.gr_number
               FROM public.gr_headers gr
              WHERE (gr.id = wi.doc_id))
            ELSE NULL::character varying
        END AS document_number,
        CASE
            WHEN (wi.doc_type = 'INV'::public.doc_type) THEN ( SELECT v.name
               FROM (public.invoice_headers ih
                 JOIN public.vendors v ON ((v.id = ih.vendor_id)))
              WHERE (ih.id = wi.doc_id))
            WHEN (wi.doc_type = 'PO'::public.doc_type) THEN ( SELECT v.name
               FROM (public.po_headers po
                 JOIN public.vendors v ON ((v.id = po.vendor_id)))
              WHERE (po.id = wi.doc_id))
            ELSE NULL::character varying
        END AS vendor_name,
    (EXTRACT(epoch FROM (now() - wi.updated_at)) / (3600)::numeric) AS hours_in_status,
        CASE
...
\unrestrict TSMMs6cGuUGZhgpSfCTElqTgbDdbcwbmdeaZhOy1tcWqiSc45YpbWmujJSjmidr
